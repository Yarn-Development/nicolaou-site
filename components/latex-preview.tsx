...
function preProcess(raw: string): string {
  ...
  // 1. Strip TikZ/PGF environments — KaTeX does not support them.
  //    Replace with a bracketed placeholder so the question text still renders
  //    and editors know a diagram was present.
  text = text.replace(
  ...
  // 2. Strip Markdown code-block wrappers (```latex ... ```)
  text = text
    .replace(/```(?:latex|math|)\s*\n?/g, '')
    .replace(/```\s*$/gm, '')

  // 3. Fix block math delimiters: \[ \] → $$
  text = text
    .replace(/\\\[/g, '$$')
    .replace(/\\\]/g, '$$')

  // 4. Fix inline math delimiters: \( \) → $
  text = text
    .replace(/\\\(/g, '$')
    .replace(/\\\)/g, '$')

  // 5. Remove conversational filler at the start
  text = text
    .replace(
  ...
  // 6. Standardize bold: **text** → \textbf{text}
  text = text.replace(/\*\*([^*]+)\*\*/g, '\\textbf{$1}')

  // 7. Fix escaped newlines: literal two-char \n → real newline
  text = text.replace(/(?<!\\)\\n/g, '\n')

  // 8. Balance unclosed $ delimiters — prevents KaTeX from consuming
