...
// =====================================================
// RAG LOGIC (exported for reuse)
// =====================================================

export function calculateRAGStatus(percentage: number): RAGStatus {
...
export function getRAGRowColor(status: RAGStatus): string {
  switch (status) {
    case "red":
...
export function useObjectiveSummaries(questions: GradedQuestion[]): ObjectiveSummary[] {
  return useMemo<ObjectiveSummary[]>(() => {
...
export function useOverallStats(questions: GradedQuestion[]) {
  return useMemo(() => {
...
export function FeedbackTable({ questions }: FeedbackTableProps) {
  return (
    <div className="border-2 border-swiss-ink overflow-x-auto">
        ...
        <tbody>
          {questions.map((question, index) => {
            ...
            return (
              <tr 
                key={question.id}
                className={`${getRAGRowColor(ragStatus)} transition-colors`}
              >
...
interface RevisionListProps {
  revisionList: ObjectiveSummary[]
  className?: string
}

export function RevisionList({ revisionList, className = "" }: RevisionListProps) {
  if (revisionList.length === 0) return null

  return (
    <div className={`border-4 border-swiss-signal bg-swiss-paper ${className}`}>
...
// =====================================================
// LEARNING OBJECTIVES SUMMARY COMPONENT
// =====================================================
...
export function ObjectivesSummary({ objectiveSummaries }: ObjectivesSummaryProps) {
  return (
...
export function FeedbackSheet({
  ...
  return (
    <div className={`${printMode ? 'print:p-8' : 'p-8'} bg-swiss-paper`}>
      ...
      {/* Learning Objectives Summary */}
      <div className="mb-8 print:mb-6">
        <ObjectivesSummary objectiveSummaries={objectiveSummaries} />
      </div>
